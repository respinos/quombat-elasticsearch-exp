...
DSN = 'mysql2://127.0.0.1:33306/dlxs?user=dlxsadm&password=a!!pri^'
...
while true
  ...
  rows.each do |row|
    ...
    browsefields.each do |field|
      query = DB[:ItemBrowse].where{Sequel.&({collid: collid}, {field: field.to_s}, {idno: idno})}
      query.each do |row|
        value = row[:value]
        if possible_maps[field].nil?
          possible_maps[field] = []
        end
        possible_maps[field] << value
      end
    end
